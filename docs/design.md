...
| 文件 | 内容 | 生命周期 | 产生方式 |
|------|------|----------|----------|
| `.token` | GitHub access_token（纯文本） | 长期有效，基本不变 | `bun run scripts/auth.js` 一次性生成 |
| `.copilot` | Copilot token exchange 返回值（JSON） | 短时有效，自动刷新 | 服务运行时自动管理 |
| `.device` | Copilot API 请求所需的设备 headers（JSON） | 固定不变 | `bun run scripts/setup-device.js` 一次性生成 |
| `.env` | 版本号等会变的配置 | 手动维护 | 手动编辑 |

### 第一步：GitHub OAuth — 独立脚本，只跑一次

运行 `bun run scripts/auth.js`，执行 GitHub Device Flow：
...
运行 `bun run scripts/setup-device.js`，生成 `.device` 文件：
...
根目录入口通过 `dotenv/config` 加载 `.env`，方便本地 Node.js 直接运行；`dist/` 构建产物会生成一个不含 `dotenv` 的入口文件，部署时依赖 Web App 自身注入环境变量。
...
├── scripts/
│   ├── auth.js                 # GitHub Device Flow OAuth → .token
│   ├── build.js                # 生成 dist/ 部署包，并写入不含 dotenv 的入口
...
bun run scripts/auth.js

# 2. 首次使用：生成设备信息（只需一次）
bun run scripts/setup-device.js
