...
| 文件 | 内容 | 生命周期 | 产生方式 |
|------|------|----------|----------|
| `.env` | GitHub token、设备 ID、版本号等所有配置 | 手动维护 | 脚本打印 + 手动粘贴 |
...
运行 `bun run scripts/setup-device.js`，打印设备 ID 环境变量：

```
VSCODE_SESSION_ID=<uuid>
VSCODE_MACHINE_ID=<uuid>
EDITOR_DEVICE_ID=<uuid>
```

手动粘贴到 `.env` 文件中。这些 ID 是 Copilot API headers 所需的设备标识，生成后固定不变。
...
**获取流程：**
1. 从 `process.env.GITHUB_TOKEN` 读取 GitHub access_token
...
- 进程重启时重新获取（仅内存缓存，无文件持久化）

**关键：`endpoints.api` 是 Copilot API 的实际 host**，请求目标 URL 从这里取，不硬编码。

### `.env` 配置

所有配置统一在 `.env` 中管理（参见 `.env.example`）：

```env
COPILOT_CHAT_VERSION=0.44.1
VSCODE_VERSION=1.116.0
GITHUB_API_VERSION=2025-10-01
PORT=4141

# GitHub token (generated by: bun run scripts/auth.js)
GITHUB_TOKEN=ghu_xxx

# Copilot device IDs (generated by: bun run scripts/setup-device.js)
VSCODE_SESSION_ID=<uuid>
VSCODE_MACHINE_ID=<uuid>
EDITOR_DEVICE_ID=<uuid>
```
...
VScode-SessionId: <VSCODE_SESSION_ID>             # 来自 .env
VScode-MachineId: <VSCODE_MACHINE_ID>             # 来自 .env
Editor-Device-Id: <EDITOR_DEVICE_ID>              # 来自 .env
...
├── scripts/
│   ├── auth.js                 # GitHub Device Flow OAuth → 打印 GITHUB_TOKEN
│   ├── build.js                # 生成 dist/ 部署包，并写入不含 dotenv 的入口
│   └── setup-device.js         # 打印设备 ID 环境变量
├── src/
...
├── .env                        # 所有配置：token、设备 ID、版本号（git ignored）
...
| Token 存储 | 内存缓存 | 运行时高效，进程重启自动重新获取 |
