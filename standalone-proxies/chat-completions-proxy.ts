...
async function proxyChatCompletions(req: Request): Promise<Response> {
  try {
    ...
    if (upstream.status !== 200) {
      ...
      console.error(`[chat-completions-proxy] upstream ${upstream.status}: ${errorBody}`);
    ...
    console.error('[chat-completions-proxy] proxy error:', error);
...
console.log(`[chat-completions-proxy] listening on http://localhost:${server.port}`);
